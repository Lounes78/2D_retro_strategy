...
class Game:
    ...
    def __init__(self):
        ...
        self.camera = Camera(800, 600,800,600)  # Taille de l'écran 800x600
    ...
    def update_sprite_counter(self):
        """Met à jour le compteur de sprite en fonction du temps écoulé."""
        flag = False
    ...
    def run_game_loop(self):
        ...
        while True:
            ...
            # Zoom controls with keys "+" and "-"
            if key_input[K_PLUS] or key_input[K_KP_PLUS]:
                self.camera.zoom_in()  # Zoom in
            elif key_input[K_MINUS] or key_input[K_KP_MINUS]:
                self.camera.zoom_out()  # Zoom out
            ...
            # Apply the zoom to the screen content (it will scale the entire screen)
            zoomed_screen = self.camera.apply_zoom(self.screen)

            # Display the zoomed screen
            self.screen.blit(zoomed_screen, (0, 0))

            pygame.display.update()
            self.poll_events_with_timeout(185)
